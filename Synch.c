...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
	...
	elseIf self.IsHeldByCurrentThread ()
		FatalError (" Logical error for requesting lock twice!")
	else
		waitingThreads.AddToEnd (currentThread)
		currentThread.Sleep ()
	endIf
	...
	if !self.IsHeldByCurrentThread ()
		FatalError ("Logical error: unlock a thread without locked")
	endIf
       	heldBy = null
	if !waitingThreads.IsEmpty ()
		...
		t.status = READY 
		readyList.AddToEnd (t) 
		heldBy = t	-- Rather tricky!, Now it helds the lock!
	endIf
        oldIntStat = SetInterruptsTo (oldIntStat)       
        endMethod
